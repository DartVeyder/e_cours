...
class SubjectSpecialtyListScreen extends Screen
{
    ...
    public function query(Subject $subject): iterable
    {
        $subjectUserSpecialties = Subject::with(['userSpecialties' => function($query) {
            $query->filters()->select('user_specialties.id', 'user_specialties.full_name', 'user_specialties.specialty', 'user_specialties.group_name','user_specialties.study_form');
        }])->find($subject->id);
